...
import { FileText, Printer, Download, FileJson } from 'lucide-react';
...
export default function ReportTemplate({
  record, onClose, title = 'Report', badgeLabel, riskTier, riskScore, opLabel,
  classificationFields, indicators, evidence, footer, jsonName,
}) {
  const reportRef = useRef(null);
  const [isPdf, setIsPdf] = useState(false);
  const [errMsg, setErrMsg] = useState(null);
  const meta = tierMeta(riskTier || record.riskTier);
  const label = badgeLabel || meta.label;

  const downloadJSON = () => {
    const blob = new Blob([JSON.stringify(record, null, 2)], { type: 'application/json' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `${jsonName || 'report'}_${record.id || 'record'}.json`;
    a.click();
    URL.revokeObjectURL(a.href);
  };

  const downloadPDF = async () => {
    setIsPdf(true); setErrMsg(null);
    try {
      ...
      pdf.save(`${jsonName || 'report'}_${record.id || 'record'}.pdf`);
    } catch { setErrMsg('PDF generation failed. Use Export JSON or Print instead.'); }
    finally { setIsPdf(false); }
  };

  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 p-3 sm:p-6" role="dialog" aria-modal="true" aria-label={title}>
      <div className="mx-auto flex min-h-full max-w-3xl flex-col">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-xl bg-navy-900 px-4 py-3 text-white">
          <div className="flex items-center gap-2">
            <FileText className="h-5 w-5 text-navy-300" />
            <span className="text-sm font-bold">{title}</span>
            <Badge color={meta.color}>{label}</Badge>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button onClick={downloadPDF} loading={isPdf}><Download className="h-4 w-4" />PDF</Button>
          </div>
        </div>
        ...
        <div ref={reportRef} className="flex-1 rounded-xl border border-slate-200 bg-white p-8 text-slate-900 shadow-xl">
          <div className="flex items-start justify-between">
            <div>
              <div className="text-xs font-bold uppercase tracking-widest text-navy-700">AegisBorder AI</div>
              <h1 className="mt-1 text-xl font-extrabold">{opLabel || record.person || 'Report'}</h1>
              <p className="text-xs text-slate-500">Record {record.id || '—'} · scanned {fmtTs(record.ts)}</p>
            </div>
            <div className="text-right">
              <div className="text-3xl font-black">{record.riskScore ?? riskScore ?? '—'}</div>
              <div className="text-xs font-semibold text-slate-500">Risk Score</div>
              <Badge color={meta.color} className="mt-1">{record.riskTier || riskTier || '—'}</Badge>
            </div>
          </div>
          ...
          <Section title="Classification">
            {classificationFields?.map((f, i) => <Field key={i} {...f} />) || [
              <Field key="op" k="Operation" v={opLabel || record.operationType || '—'} />,
              <Field key="cls" k="Classification" v={record.classification} />,
              <Field key="id" k="Target / identifier" v={record.documentNumber} mono />,
              <Field key="dec" k="Decision" v={record.decision} />,
              <Field key="wl" k="Watchlist flag" v={record.watchlistFlagged ? 'Yes' : 'No'} />,
            ]}
          </Section>
          <Section title="Indicators">
            {(indicators || record.factors || []).length > 0 ? (
              <ul className="space-y-1.5">
                {(indicators || record.factors || []).map((f, i) => (
          ...
          <Section title="Analysis detail">
            {(evidence || record.evidence || []).map((e, i) => (
              <div key={i} className="mb-1.5 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700">{e}</div>
            ))}
          </Section>
          <div className="mt-6 flex items-center gap-1.5 text-xs text-slate-400">
            <FileText className="h-3.5 w-3.5" /> {footer || 'On-device analysis · Report generated by AegisBorder AI · Record retained in this session only'}
          </div>
        </div>
      </div>
    </div>
